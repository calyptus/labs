...
var group = new ART.Group().inject(art).scale(2);
...
new ART.Ellipse(200, 100).fillRadial(['#000', '#F00', '#FFF'], .75, .5, 1, .575, .5).translate(500, 200).inject(group);
...
size = new ART.Shape(p).fill('#00f', '#f00', 'rgba(255, 0, 0, .2)', '#0f0').translate(150, 310).stroke('#000').inject(group).measure();
...
rt.fillRadial({ 0.0: '#000', '1.0': '#FFF' }, .5, 1, .5, .5, .5);
...
setInterval(function(){
	...
	//gt.fillLinear(['#000', '#00f', '#f00', 'rgba(0, 255, 0, 0)', '#FFF'], angle);
	//gt.fillRadial({ 0.0: '#000', '1.0': '#FFF' }, .25, .5, .5, 1, 1);
	
	// Linear gradient Math test
	var angle = (new Date() % 30000) / 30000 * 360;
	gt.fillLinear(['#000', '#F00', 'rgba(255, 255, 255, 0)'], angle);
